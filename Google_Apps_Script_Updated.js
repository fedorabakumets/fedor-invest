...
function getPaymentData() {
    ...
    rawData.forEach((row, index) => {
        const payment = escapeValue(row[0]);
        const url = escapeValue(row[1]);
        if (payment && url) {
            result.push({
                payment: payment.toLowerCase(),
                url: url
            });
            console.log(`getPaymentData: Строка ${index + 2}: ${payment} -> ${url}`);
        }
    });
...
function onEdit(e) {
    ...
    changesSheet.appendRow([timestamp, site, parameter, oldValue, newValue]);
